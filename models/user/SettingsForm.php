 ...
 */
class SettingsForm extends Model implements UserEventsInterface
{
  use ModuleTrait;
  ...
  /** @inheritdoc */
  public function __construct(User $user = null, Mailer $mailer, $config = [])
  {
    $this->_user = $user;
    $this->mailer = $mailer;
    $this->setAttributes([
  ...
  public function rules()
  {
    $rules = [
        ...
        'currentPasswordLength' => ['current_password', 'string', 'max' => 72, 'min' => 6],
    ];
    if (!(Yii::$app->user->can('createUser'))) {
      $rules['currentPasswordRequired'] = ['current_password', 'required'];
      $rules['currentPasswordValidate'] = ['current_password', function ($attr) {
        if (!Password::validate($this->$attr, $this->user->password_hash)) {
          $this->addError($attr, Yii::t('user', 'Current password is not valid'));
          Yii::$app->session->setFlash('wrong_passwrd', 'Вы ввели неправильный пароль!');
        }
      }];
    }
    return $rules;
   ...
   * @return bool
   */
  public function save($oldEmail = null)
  {
    $eventsController = new EventController('User', [UserEventsInterface::EVENT_ON_PASSWORD_CHANGED,
        UserEventsInterface::EVENT_ON_EMAIL_CHANGED,
        UserEventsInterface::EVENT_ON_LOGIN_CHANGED]);

    if ($this->validate()) {
      $this->user->scenario = 'settings';

      $user = User::find()->where(['email' => $oldEmail])->one()->id;
      $oldUsername = $this->user->username;

      if ($oldUsername != $this->username) {
        $event = new UserEvent(User::getRoleName($user)['item_name'], $this->username, $this->username, UserEventsInterface::EVENT_ON_LOGIN_CHANGED, 'USER_EVENT');
      ...
      if ($this->email == $this->user->email && $this->user->unconfirmed_email != null) {
        ...
        switch ($this->module->emailChangeStrategy) {
          case Module::STRATEGY_INSECURE:
            ...
            $event = new UserEvent(User::getRoleName($user)['item_name'], $oldUsername, $oldUsername, UserEventsInterface::EVENT_ON_EMAIL_CHANGED, 'USER_EVENT');
            $eventsController->triggerEvent($event->event_name, $event);
      ...
      if (($this->new_password != null || $this->new_password != "") && $this->user->save()) {

        $event = new UserEvent(User::getRoleName($user)['item_name'], $oldUsername, $oldUsername, UserEventsInterface::EVENT_ON_PASSWORD_CHANGED, 'USER_EVENT');
        $eventsController->triggerEvent($event->event_name, $event);
  ...
  {
    $this->mailer->sendUserPasswordChanged($event->registeredUser);
  }
